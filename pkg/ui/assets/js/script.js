...
function controlBackgroundStyle() {
  ...
  if (backgroundStyle.value == "solid") {
    ...
    buttonBackground.style.backgroundColor = document.getElementById(
      "backgroundColourControl"
    ).value;
    buttonBackground.style.backgroundImage = "none";
    localStorage.setItem(
      "backgroundColour",
      document.getElementById("backgroundColourControl").value
    );
  }

  if (backgroundStyle.value == "gradient") {
    document.getElementById("backgroundSolidOptions").style.display = "none";
    document.getElementById("backgroundGradientOptions").style.display =
      "block";
    document.getElementById("backgroundImageOptions").style.display = "none";

    backgroundColour1 = document.getElementById(
      "backgroundColour1Control"
    ).value;
    backgroundColour2 = document.getElementById(
      "backgroundColour2Control"
    ).value;
    ...
    if (backgroundGradient == "horizontal") {
      buttonBackground.style.backgroundImage =
    ...
    if (backgroundGradient == "radial") {
      buttonBackground.style.backgroundImage =
        "radial-gradient(" + backgroundColour1 + ", " + backgroundColour2 + ")";
    }
  ...
  if (backgroundStyle.value == "image") {
    ...
    if (document.getElementById("backgroundImageControl").files.length != 0) {
      ...
      if (FileReader && files && files.length) {
        ...
        fr.onload = function () {
          document.getElementById("buttonBackground").style.backgroundImage =
            "url(" + fr.result + ")";
        };
...
function controlIconType(fa = null, md = null) {
  ...
  if (iconType.value == "none") {
    let hideIconOptions = document.querySelectorAll(
      ".charIconOptions, .faIconOptions, .uploadIconOptions, .mdIconOptions"
    );
  ...
  if (iconType.value == "fa") {
    let hideIconOptions = document.querySelectorAll(
      ".charIconOptions, .uploadIconOptions, .mdIconOptions"
    );
    hideIconOptions.forEach((option) => {
      option.style.display = "none";
    });
    ...
    document.querySelector("#buttonIconInner i").style.fontSize =
      iconSize.value;
  ...
  if (iconType.value == "md") {
    let hideIconOptions = document.querySelectorAll(
      ".charIconOptions, .uploadIconOptions, .faIconOptions"
    );
    hideIconOptions.forEach((option) => {
      option.style.display = "none";
    });
    ...
    document.querySelector("#buttonIconInner span").style.fontSize =
      iconSize.value;
    buttonIconInner.style.color = iconColour.value;
  }
  ...
  if (iconType.value == "char") {
    let hideIconOptions = document.querySelectorAll(
      ".faIconOptions, .uploadIconOptions, .mdIconOptions"
    );
    hideIconOptions.forEach((option) => {
      option.style.display = "none";
    });
...
function iconSearch(elem) {
  ...
  var hits = document.querySelectorAll(
    `#iconPicker-fa .icon[data-search*='${query}'], #iconPicker-md  .icon[data-search*='${query}']`
  );
  var icons = document.querySelectorAll(
    `#iconPicker-fa .icon, #iconPicker-md .icon`
  );
...
function init() {
  ...
  controls.forEach(function (item, index, arr) {
    if (localStorage.getItem(item)) {
      document.getElementById(item + "Control").value =
  ...
  document.getElementById("textFontControl").addEventListener(
    "showDropdown",
    function () {
      const fontChoices = document.querySelectorAll(
        "[id^=choices--textFontControl-item-choice]"
      );
...
document.getElementById("download").addEventListener("click", function () {
  htmlToImage
    .toPng(document.getElementById("preview-button"), {
      quality: 1,
      height: 244,
      width: 244,
    })
